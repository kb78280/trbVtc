...
type FormData = {
  ...
  // Méthode de paiement
  methodePaiement?: 'immediate' | 'sur-place'
}
...
export default function SecureReservationForm() {
  ...
  // État pour le wizard multi-étapes
  ...
  // Fonction pour calculer le prix selon les nouvelles règles
  ...
  }, [serviceType, routeInfo, watch])

  // Recalculer le prix quand les paramètres changent
  useEffect(() => {
    const price = calculatePrice()
    setPriceBreakdown(price)
  }, [calculatePrice, routeInfo, watch('duree'), watch('methodePaiement')])

  // Fonction pour obtenir le prix total (pour compatibilité)
  const calculateEstimatedPrice = () => {
    return priceBreakdown?.totalTTC || 0
  }
  ...
  const canProceedToNextStep = () => {
    switch (currentStep) {
      case 1: // Service & Véhicule
        return serviceType && vehicleType
      case 2: // Trajet
        if (serviceType === 'transfert') {
          return departValueRef.current.trim() && arriveeValueRef.current.trim()
        } else {
          return departValueRef.current.trim() && watch('duree')
        }
      case 3: // Aperçu & Options
        return priceBreakdown && priceBreakdown.totalTTC > 0
      case 4: // Infos & Paiement
        return watch('prenom') && watch('nom') && watch('telephone') && watch('email')
      default:
        return true
    }
  }
  ...
  // Étapes du wizard
  const steps = [
    { number: 1, title: 'Service', subtitle: 'Type de trajet' },
    { number: 2, title: 'Trajet', subtitle: 'Détails du voyage' },
    { number: 3, title: 'Aperçu', subtitle: 'Prix et options' },
    { number: 4, title: 'Validation', subtitle: 'Informations et paiement' }
  ]
    ...
    <section id="reservation" className="py-12 bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl">
          ...
          {/* Contenu des étapes du wizard */}
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6 min-h-[500px]">
            
            {/* Étape 1: Choix du service et du véhicule */}
            {currentStep === 1 && (
              <div className="space-y-6">
                <div>
                  <h4 className="text-lg font-medium text-gray-900 mb-4">Quel type de service souhaitez-vous ?</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() => handleServiceTypeChange('transfert')}
                      disabled={rateLimitInfo.blocked}
                      className={`p-6 rounded-lg border-2 text-center transition-colors disabled:opacity-50 ${
                        serviceType === 'transfert'
                    ...
                    >
                      ...
                      <div className="text-xs mt-2 text-gray-500">Idéal pour aéroports, gares...</div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="text-xs mt-2 text-gray-500">Pour vos rendez-vous, visites...</div>
                    </button>
                  </div>
                </div>

                <div>
                  <h4 className="text-lg font-medium text-gray-900 mb-4">Quel véhicule préférez-vous ?</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="text-xs mt-2 text-blue-600 font-medium">
                        {serviceType === 'transfert' ? '1,49€/km' : '65€/h'}
                      </div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <div className="text-2xl mb-2">🚐</div>
                      <div className="font-semibold">Van Confort</div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div>
                  <h4 className="text-lg font-medium text-gray-900 mb-4">
                    {serviceType === 'transfert' ? 'Où souhaitez-vous aller ?' : 'Où souhaitez-vous être pris en charge ?'}
                  </h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          {...register('passagers', { required: 'Le nombre de passagers est requis' })}
                          className="w-full p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                        >
                          {Array.from({ length: vehicleType === 'van' ? 8 : 3 }, (_, i) => (
            ...
            {/* Étape 3: Aperçu et Options */}
            {currentStep === 3 && (
              <div className="space-y-6">
                <div>
                  <h4 className="text-lg font-medium text-gray-900 mb-4">Récapitulatif de votre réservation</h4>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-600">{serviceType === 'transfert' ? 'Transfert' : 'Mise à disposition'}</p>
                        <p className="text-gray-600">{vehicleType === 'van' ? 'Van (1-8 passagers)' : 'Voiture (1-3 passagers)'}</p>
                      </div>
                      <div>
                        <h5 className="font-medium text-gray-900">Trajet</h5>
                        <p className="text-gray-600">Départ: {departValueRef.current || 'Non défini'}</p>
                        {serviceType === 'transfert' ? (
                          <p className="text-gray-600">Arrivée: {arriveeValueRef.current || 'Non défini'}</p>
                        ) : (
                          <p className="text-gray-600">Durée: {watch('duree')} heure(s)</p>
                        )}
                      </div>
                    </div>
                  </div>

                  {/* Calcul de prix détaillé */}
                  {priceBreakdown && (
                    <div className="bg-white border border-gray-200 rounded-lg p-4">
                      <h5 className="font-medium text-gray-900 mb-3">Détail du prix</h5>
                      
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span>Prix HT ({serviceType === 'transfert' ? `${priceBreakdown.distance} km` : `${priceBreakdown.duration} h`}):</span>
                        ...
                        <div className="border-t pt-2 flex justify-between font-medium">
                          <span>Total TTC:</span>
                          <span className="text-blue-600">{priceBreakdown.totalTTC.toFixed(2)} €</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    <h5 className="font-medium text-gray-900 mb-3">Options supplémentaires</h5>
                    <div className="space-y-3">
                      <label className="flex items-center">
                        ...
                        />
                        <span className="ml-2 text-sm">Bouquet de fleurs (+25€)</span>
                      </label>
                      <label className="flex items-center">
                        <input
                          type="checkbox"
                          ...
                          className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        />
                        <span className="ml-2 text-sm">Assistance aéroport (+20€)</span>
                      </label>
                      <div>
            ...
            {/* Étape 4: Informations personnelles et paiement */}
            {currentStep === 4 && (
              <div className="space-y-6">
                <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label className="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                      ...
                      />
                      <div className="ml-3">
                        <div className="font-medium">Paiement immédiat</div>
                        <div className="text-sm text-gray-600">Par carte bancaire</div>
                      </div>
                    </label>
                    <label className="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                      <input
                      ...
                      <div className="ml-3">
                        ...
                        <div className="text-sm text-gray-600">Au chauffeur</div>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                {watch('methodePaiement') === 'immediate' && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h5 className="font-medium text-gray-900 mb-3">Paiement sécurisé</h5>
                    {((serviceType === 'transfert' && departValueRef.current.trim() && arriveeValueRef.current.trim()) ||
                      (serviceType === 'mise-a-disposition' && watch('duree'))) && 
                     priceBreakdown && priceBreakdown.totalTTC > 0 ? (
                      <StripePaymentForm
                        amount={Math.round(priceBreakdown.totalTTC * 100)}
                        onSuccess={() => {
                      ...
                      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-yellow-800 text-sm">
                          Veuillez compléter les étapes précédentes pour activer le paiement.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm">
              ...
              <p className="text-sm text-gray-600 mb-6">Choisissez votre mode de paiement préféré</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          <h4 className="font-medium text-gray-900">Paiement immédiat</h4>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                          Payez maintenant par carte bancaire de manière sécurisée
                        </p>
                        <div className="mt-2">
                ...
                <label className="cursor-pointer">
                  ...
                  />
                  <div className={`p-4 border-2 rounded-lg transition-all ${
                    watch('methodePaiement') === 'sur-place'
                      ? 'border-blue-600 bg-blue-50'
                      : 'border-gray-300 bg-white hover:border-gray-400'
                  }`}>
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-gray-900">Paiement sur place</h4>
                        <p className="text-sm text-gray-500 mt-1">
                          Payez directement dans le véhicule
                        </p>
                        <div className="mt-2 space-y-1">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-gray-600">Paiement sans contact (téléphone)</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {watch('methodePaiement') === 'immediate' && (
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div onClick={(e) => e.preventDefault()}>
                    {/* Condition pour afficher Stripe : adresses remplies pour transfert OU durée définie pour mise à disposition */}
                    {((serviceType === 'transfert' && departValueRef.current.trim() && arriveeValueRef.current.trim()) ||
                      (serviceType === 'mise-a-disposition' && watch('duree'))) && 
                     priceBreakdown && priceBreakdown.totalTTC > 0 ? (
                      <StripePaymentForm
                      ...
                      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-yellow-800 text-sm">
                          {serviceType === 'transfert' 
                            ? 'Veuillez d\'abord renseigner les adresses de départ et d\'arrivée pour calculer le prix et activer le paiement.'
                            : 'Veuillez d\'abord sélectionner la durée souhaitée pour calculer le prix et activer le paiement.'
                          }
                        </p>
                      </div>
                    )}
              ...
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-sm text-blue-800">
                    ...
                    <p className="mt-1">
                      Le paiement immédiat bénéficie d'une réduction de 5% et sécurise votre réservation. 
                      Le paiement sur place accepte les cartes bancaires et le paiement sans contact avec votre téléphone.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {priceBreakdown && (
              <div className="bg-white p-6 rounded-lg shadow-sm">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Détail du prix</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div className="space-y-2">
                    ...
                    {serviceType === 'transfert' && priceBreakdown.distance && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Distance:</span>
                        <span className="font-medium">{priceBreakdown.distance} km</span>
                      </div>
                    )}
                    {serviceType === 'mise-a-disposition' && priceBreakdown.duration && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Durée:</span>
                        <span className="font-medium">{priceBreakdown.duration} h</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="border-t border-gray-200 pt-4 space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-600">TVA ({serviceType === 'transfert' ? '10' : '20'}%):</span>
                    <span className="font-medium">{priceBreakdown.tva.toFixed(2)} €</span>
                  </div>
                  
                  {priceBreakdown.stripeFees > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Frais de traitement Stripe:</span>
                      <span className="font-medium">{priceBreakdown.stripeFees.toFixed(2)} €</span>
                    </div>
                  )}
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-green-600">
                        {priceBreakdown.totalTTC.toFixed(2)} €
                      </span>
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    {serviceType === 'transfert' 
                      ? 'Prix basé sur la distance calculée par Google Maps' 
                      : 'Prix basé sur la durée sélectionnée'
                    }
                  </p>
                </div>
              </div>
            ...
            >
              {isSubmitting ? (
                ...
                <span className="flex items-center justify-center">
                  ...
                  Payer maintenant ({priceBreakdown?.totalTTC.toFixed(2) || '0.00'} €)
                </span>
              ) : (
                'Réserver et voir le prix'
              )}
            </button>
